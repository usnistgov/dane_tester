...
end-to-end encryption of the individual email messages, and
point-to-point encryption of the communications between email
servers. Because of the scale of the Internet, securing such systems
with a symmetric encryption is unfeasible; only security systems based
on public key
cryptography can be adequately scaled to match the task.

Public key cryptography requires that participants have some mechanism
...
keys, and PKI can be used to certify them. 
...
associated with DNS-named. By relying on the DNSSEC\cite{rfc3833}
...
global directory for certified identities.

Support for DANE has been added to OpenSSL (v. 1.1.0) and GnuTLS
(v.3.5.4). But properly implementing DANE requires not just conforming
software, but the generation and installation of X.509 certificates
and/or PGP keys, the generation and installation of appropriate DNS
records, and validating DNSSEC records. A mismatch of any of these
records might result in the inability to properly encrypt, sign,
validate, or deliver email. Thus, there is a significant need for an
easy-to-use testing infrastructure, and this testing infrastructure
...
as well as certificates that are self-signed. Since mail is sent by default without encryption, this
...
open to interception by an active attacker that can perform a
man-in-the-middle attack. Such attacks can be readily performed by
manipulating the DNS.

RFC7671 and RFC7672 provide a mechanism for SMTP clients to discover
information that allows the certification of an SMTP server's public
key\cite{rfc7671,rfc7672}. Thus, while DANE cannot prevent
man-in-the-middle attacks, it provides a technique that an MTA can use
to determine that a man-in-the-middle attack is taking place. Once
detected, the server can refuse to send email.

Support for RFC 7672 has been incorporated into Postfix (v2.11).
...
(e.g. \cite{rfc1421,rfc4880,rfc5750}) has produced two incompatible
standards have emerged for the exchange of encrypted Internet email:
S/MIME and OpenPGP. Both of these standards require the sender of an
encrypted message to first obtain the public key of the message's
intended recipient. Thus, they both suffer from the \emph{key
  discovery problem}. However, S/MIME mitigates this problem somewhat,
as digitally signed S/MIME messages typically have the sender's public
key embedded in the PKCS\#7 attachment\cite{rfc2315}.

Work is in progress on a mechanism for discovering S/MIME certificates
associated with email addresses (\emph{draft-ietf-dane-smime11}, and
an experimental RFC was recently approved for associating OpenPGPKey
...
either an HTTP or SMTP server. The tester then obtains the service
certificate for the HTTP or SMTP server and certificate and verifies
the certificate against the RR(s). The tester can be run from
...
difficult to develop, audit, and extend to new protocols. A 
...
and configuration changes. Thus, it is sometimes difficult to determine the
...
and audited through a system.'' A design requirement of this tester  was to maintain a log of all DNS queries and responses
...
directed by the responses from the systems undergoing test. Our second
tester makes use of this observation to result in tests that are
decomposed into components, making them easy to validate and audit.
...
In 2009 Fan et al.\ developed a protocol tester based on Finite State Machine
...
testing web services\cite{Bertolino:2009:ASB:1595696.1595719}. Pike presented an approach for building protocol testers using the
Haskell programming language.\cite{Pike:2009:RYO:1596638.1596646}

In 2011 Wang et al.\ discussed automated approaches for bringing testers to rarely
...
% LocalWords:  inavailability unencrypted repeatability
