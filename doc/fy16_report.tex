...
\section{SMIMEA and OpenPGPKey Tester}
...
Based on our experience in developing and using the server tester, we
had additional requirements on the test infrastructure as well:
...
Given these requirements, we decided to create a new framework for the
tester:

\begin{itemize}
\item Instead of controlling the test progression with a series of
  |if|, |for| and |while| statements, the new tester would implement each
...
\item Each test is given a unique name and identifier. The test
...
file & purpose\\
\hline
\multicolumn{1}{l}{\textbf{HTML files}}\\
...
\\
\multicolumn{1}{l}{\textbf{Python Modules and Scripts}}\\
...
\\
\multicolumn{1}{l}{\textbf{CGI Scripts}}\\
...
\subsection{System Preparation}
\subsection{Installation and Operation}
A user needs to be configured to run the scripts. As installed on
|dane-test|, the |slg| user runs the tester.  The following line added
to the user's cron script (e.g. ``crontab -e''):

\begin{Verbatim}
#
# m h  dom mon dow   command
* * * * * dane_tester/openpgp_smimea/cron_runner.py
\end{Verbatim}
