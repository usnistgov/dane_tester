#!/usr/bin/env python3
#
# Figure out why https://good-pkixta.dane.huque.com/ doesn't verify
...
if __name__=="__main__":
    import os
    import hashlib
    qhost = "good-pkixta.dane.huque.com"
    port = 443
    protocol = 'https'
    ret = dns_query_tlsa(qhost,port)
    print("TLSA Records for {}: {}".format(qhost,len(ret)))
    ip_results = dns_query(qhost)
    print("IP addresses for {}: {}".format(qhost,len(ip_results)))
    for r in ret:
        usage = r.rr.usage
        selector = r.rr.selector
        mtype = r.rr.mtype
        print(r)
        print("Usage: {} Selector: {} Mtype: {}".format(usage,selector,mtype))
        for ip_result in ip_results:
            ipaddr = ip_result.data
            print("Checking IP Address: {}".format(ipaddr))
            print("Fetching certificates for {}:{}".format(qhost,port))
            cert_chain = openssl_get_service_certificate_chain(ipaddr,qhost,port,'https')[0].data

            # If usage==0, see if we can find the appropriate CA certificate
            if usage==0:
                cacert = get_cafile_certificate_by_tlsa_rr(r.rr)
                if cacert:
                    print(" **** FOUND CA ****")
                    # Now convert each cert to DER format and check the hash
                    sel = tlsa_cert_select(selector,cacert)
                    hasher = hashlib.sha256()
                    hasher.update(sel)
                    print("SHA256(DER SELECTED)={}".format(hasher.hexdigest()))
                    print("cacert:\n",cacert)
                    print("cert_chain:\n",cert_chain)
                    r = cert_verify(cacert,cert_chain,[qhost],ipaddr,r.rr.usage)
                    print("cert_verify:",r)
                    print("count_passed=",count_passed(r,False))
